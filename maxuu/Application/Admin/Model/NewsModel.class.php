...
class NewsModel extends Model {
	...
	protected $_validate = array(
		...
		array('desc','1,500','导语长度不合法（1-500个字符）',self::MUST_VALIDATE,'length'),
		array('source','1,30','出处长度不合法（1-30个字符）',self::MUST_VALIDATE,'length'),
		array('url','1,255','来源长度不合法（1-255个字符）',self::MUST_VALIDATE,'length'),
	...
	public function getList($type='news',$cids=0,$p=0){
		...
		$field = 'c.name as categorynews_name,e.category_id,e.id,e.name,e.desc,e.keywords,e.on_sale,e.recommend,e.add_time,e.source,e.url,e.links';
	...
	public function getGoods($where){
		//定义需要的字段
		$field = 'id,category_id,sn,name,thumb,on_sale,content,recommend,desc,keywords,add_time,source,url,links';
