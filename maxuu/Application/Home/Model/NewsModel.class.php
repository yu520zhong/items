...
class NewsModel extends Model {
	...
	public function getList($cids=0,$p=0) {
		$field = 'category_id,id,name,desc,add_time,author,content,thumb,source,url,click';
		//查找分类ID数据
		if($cids > 0) {
	...
	public function postList($cids=0,$p=0) {
		$field = 'c.name as categorynews_name,a.category_id,a.id,a.name,a.desc,a.thumb,a.sn,a.add_time,a.content,a.source,a.url,a.click';
		//查找分类ID数据
		if($cids > 0) {
	...
	public function getDate($where) {
		$field = 'id,category_id,name,desc,thumb,add_time,content,source,url,click';
		//处理排序条件
		$order = 'id desc';
		//查询文章数据
